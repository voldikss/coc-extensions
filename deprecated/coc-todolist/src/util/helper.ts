...
export async function createTodoEditBuffer(
  ...
  nvim.command(
    `setl filetype=coc_todolist buftype=acwrite nobuflisted bufhidden=wipe nofen wrap`,
    true,
  )
  nvim.command(
    `setl undolevels=1000 nolist nospell noswapfile nomod conceallevel=2 concealcursor=n`,
    true,
  )
