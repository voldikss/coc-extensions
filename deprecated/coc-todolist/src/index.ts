...
export async function activate(context: ExtensionContext): Promise<void> {
  ...
  subscriptions.push(commands.registerCommand('todolist.create', async () => await todo.create()))

  subscriptions.push(commands.registerCommand('todolist.upload', async () => await todo.upload()))

  subscriptions.push(
    commands.registerCommand('todolist.download', async () => await todo.download(storagePath)),
  )

  subscriptions.push(commands.registerCommand('todolist.export', async () => await todo.export()))

  subscriptions.push(commands.registerCommand('todolist.clear', async () => await todo.clear()))

  subscriptions.push(
    commands.registerCommand('todolist.gist.genToken', async () => {
      await oauth.start()
    }),
  )

  subscriptions.push(
    commands.registerCommand('todolist.gist.openBrowser', async () => {
  ...
  subscriptions.push(listManager.registerList(new TodoList(nvim, db)))
}
